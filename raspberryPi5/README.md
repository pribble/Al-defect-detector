...
| 服务 | 目录 | 端口 | 说明 |
|------|------|------|------|
| main（Flask） | `main/` | 7777 | 检测 + 机械臂分拣合一：相机采集 + SSIM 触发 + FPGA 推理客户端 + SQLite 记录 + MJPEG 视频流 + 5-DOF 机械臂 OK/NG 分拣（队列化串行执行，串口 9600） |
| nginx | `frontend/` | 80 / 8080 | 托管前端 SPA |
| systemd | — | — | `detect-api.service`（唯一单元；原 `api.service` 已随服务合并废弃） |

> 原 GrabImage（:7777 检测）与 ArmControl（:8899 机械臂）两个服务已合并为
> `main/` 单进程单端口；检测结果触发机械臂改为本地入队（`arm_control.enqueue_grab`），
> 不再 HTTP 自调用。详见 [main/README.md](main/README.md)。
...
├── tools/        # crop_train_dataset.py（训练集方形化预处理，复用 main/disc_detect.py）
├── deploy.sh     # rsync 部署脚本（dev 机 → Pi）
└── restart.sh    # 重启主服务并查看状态
```

> `tools/logger.py` 已移入 `main/logger.py`（与主服务同目录，随服务部署）。
...
`frontend/config.json` 配置后端地址（服务合并后 `ip`/`url` 均为 :7777）：

```json
{"ip": "http://172.16.68.111:7777", "url": "http://172.16.68.111:7777", "title": "工业缺陷检测管理平台", "version": "v4.0"}
```

功能：实时 MJPEG 画面、检测统计（缺陷分布饼图 + 当月趋势）、手动机械臂控制、
传送带速度标定、历史记录与缺陷图像浏览。视频流源：
`http://172.16.68.111:7777/img_disc`（带圆检测叠加：绿圆 + 黄裁剪框；原始流
`/img` 保留备用）。机械臂 API（`/use_arm`、`/get_arm`）与检测 API 同端口。
