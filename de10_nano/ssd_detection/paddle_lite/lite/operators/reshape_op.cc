...
bool ReshapeOp::AttachImpl(const cpp::OpDesc &opdesc, lite::Scope *scope) {
  param_.x =
      scope->FindVar(opdesc.Input("X").front())->GetMutableTensor();
  CHECK(param_.x);
  param_.output =
      scope->FindVar(opdesc.Output("Out").front())->GetMutableTensor();
  CHECK(param_.output);
  ...
  if (opdesc.HasInput("ShapeTensor") && !opdesc.Input("ShapeTensor").empty()) {
    ...
    for (auto arg : args) {
      ...
      if (var != nullptr) {
        param_.shape_tensor_vct.push_back(var->GetMutableTensor());
      }
    }
  ...
  if (opdesc.HasInput("Shape") && !opdesc.Input("Shape").empty()) {
    ...
    if (var != nullptr) {
      param_.shape_tensor = var->GetMutableTensor();
    }
  }
...
bool Reshape2Op::AttachImpl(const cpp::OpDesc &opdesc, lite::Scope *scope) {
  ...
  param_.xshape = xshape_var->GetMutableTensor();
  CHECK(xshape_var);
  return true;
}
