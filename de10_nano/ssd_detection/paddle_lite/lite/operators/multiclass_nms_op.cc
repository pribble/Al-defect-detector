...
bool MulticlassNmsOpLite::AttachImpl(const cpp::OpDesc& opdesc,
  ...
  if (std::find(output_arg_names.begin(), output_arg_names.end(), "Index") !=
    ...
    param_.index = GetMutableVarTensor(scope, index_name);
  }
  param_.bboxes = GetVarTensor(scope, bboxes_name);
  param_.scores = GetVarTensor(scope, scores_name);
  param_.out = GetMutableVarTensor(scope, out_name);
  ...
  if (std::find(input_arg_names.begin(), input_arg_names.end(), "RoisNum") !=
    ...
    if (rois_num_name.size() > 0) {
      param_.rois_num = GetVarTensor(scope, rois_num_name.front());
    }
  }
  ...
  if (std::find(output_arg_names.begin(),
    ...
    param_.nms_rois_num = GetMutableVarTensor(scope, nms_rois_name);
  }
