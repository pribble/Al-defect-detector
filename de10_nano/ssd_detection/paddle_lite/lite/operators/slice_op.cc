...
bool SliceOp::AttachImpl(const cpp::OpDesc &opdesc, lite::Scope *scope) {
  ...
  bool input_is_array = input_var->IsTensorList();
  bool out_is_array = output_var->IsTensorList();
  if (input_is_array) {
    param_.XTensorList = input_var->GetMutableTensorList();
  ...
  if (out_is_array) {
    param_.OutTensorList = output_var->GetMutableTensorList();
    ...
    if (!StartsTensorList.empty() &&
      ...
      for (auto var : StartsTensorList) {
        param_.StartsTensorList.push_back(
            scope->FindVar(var)->GetMutableTensor());
      }
    }
