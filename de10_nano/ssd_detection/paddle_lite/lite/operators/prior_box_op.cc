...
bool PriorBoxOpLite::AttachImpl(const cpp::OpDesc& opdesc, lite::Scope* scope) {
  ...
  param_.input = scope->FindVar(input)->GetMutableTensor();
  param_.image = scope->FindVar(image)->GetMutableTensor();
  param_.boxes = scope->FindVar(boxes)->GetMutableTensor();
  param_.variances = scope->FindVar(variances)->GetMutableTensor();
