...
class ConvOpLite : public OpLite {
 public:
  ...
  bool AttachImpl(const cpp::OpDesc& op_desc, lite::Scope* scope) override {
    ...
    param_.x = scope->FindVar(X)->GetMutableTensor();
    param_.filter = scope->FindVar(Filter)->GetMutableTensor();
    param_.output = scope->FindVar(Out)->GetMutableTensor();
    CHECK(param_.x);
    CHECK(param_.filter);
    CHECK(param_.output);
    ...
    if (std::find(input_arg_names.begin(), input_arg_names.end(), "Bias") !=
      ...
      if (bias_arguments.size() > 0) {
        ...
        if (bias_var != nullptr) {
          param_.bias =
              const_cast<lite::Tensor*>(&(bias_var->GetTensor()));
        }
      }
    }
    ...
    if (op_desc.HasAttr("with_act") && op_desc.GetAttr<bool>("with_act")) {
      ...
      if (act_type == "relu") {
        ...
        param_.activation_param.Prelu_alpha =
            const_cast<lite::Tensor*>(&(prelu_alpha_var->GetTensor()));
      } else {
    ...
    if (op_desc.HasAttr("fuse_elementwise_op_type")) {
      ...
      param_.second_x =
          const_cast<lite::Tensor*>(&(scope->FindVar(X)->GetTensor()));
    }
